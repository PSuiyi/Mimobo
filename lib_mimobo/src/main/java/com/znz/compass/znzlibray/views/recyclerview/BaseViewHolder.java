...
public class BaseViewHolder extends RecyclerView.ViewHolder {
    ...
    public BaseViewHolder(final View view, Context mContext) {
        super(view);
     ...
     */
    public BaseViewHolder setText(@IdRes int viewId, @NonNull CharSequence value) {
        TextView view = getView(viewId);
        view.setText(value);
        return this;
    }

    public BaseViewHolder setTextWithVip(@IdRes int vipId, @IdRes int viewId, CharSequence value) {
        TextView vip = getView(vipId);
        TextView view = getView(viewId);
        int width = (int) StringUtil.getTextViewLength(vip, vip.getText().toString().trim());// 计算前面字体宽度的像素
        SpannableStringBuilder ssb = new SpannableStringBuilder(value);
        ssb.setSpan(new LeadingMarginSpan.Standard(width + DipUtil.dip2px(mContext, 10), 0), 0, ssb.length(), Spanned.SPAN_INCLUSIVE_EXCLUSIVE);// 设置首行缩进
        view.setText(ssb);
