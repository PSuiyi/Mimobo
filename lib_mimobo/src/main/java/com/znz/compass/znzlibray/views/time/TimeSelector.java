package com.znz.compass.znzlibray.views.time;
...
import com.znz.compass.znzlibray.views.time.view.PickerView;
...
public class TimeSelector {
    ...
    private boolean isAfterNow;//设置能够选择的时间范围是否是当前时间之后
    private boolean isDefaultNow;//设置初始的时间是否是当前时间
    ...
    private String FORMAT_OUT = "yyyy-MM-dd";
    ...
    private void initView() {
        ...
        tvYear = (TextView) seletorDialog.findViewById(R.id.tvYear);
        tvMonth = (TextView) seletorDialog.findViewById(R.id.tvMonth);
        ...
        tv_select.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                if (isAfterNow) {
                    if (selectedCalender.getTime().getTime() <= System.currentTimeMillis()) {
                        DataManager.getInstance(context).showToast("选择的时间不能小于现在的时间");
                        handler.handle("error");
                    } else {
                        handler.handle(DateUtil.format(selectedCalender.getTime(), FORMAT_OUT));
                    }
                } else {
                    handler.handle(DateUtil.format(selectedCalender.getTime(), FORMAT_OUT));
                }
    ...
    private void loadComponent() {
        ...
        if (isDefaultNow) {
            ...
            now.setTime(DateUtil.parse(TimeUtils.getNowTimeString(FORMAT_STR), FORMAT_STR));
            year_pv.setSelected(fomatTimeUnit(now.get(Calendar.YEAR)));
            month_pv.setSelected(fomatTimeUnit(now.get(Calendar.MONTH) + 1));
            day_pv.setSelected(fomatTimeUnit(now.get(Calendar.DAY_OF_MONTH)));
            hour_pv.setSelected(fomatTimeUnit(now.get(Calendar.HOUR_OF_DAY)));
            minute_pv.setSelected(fomatTimeUnit(now.get(Calendar.MINUTE)));
        } else {
    ...
    public void setIsAfterNow(boolean b) {
        isAfterNow = b;
    }

    public void setDefaultTimeNow(boolean defaultNow) {
        isDefaultNow = defaultNow;
    }

    public void setFormatDHM() {
