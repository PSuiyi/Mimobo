 ...
 */
package com.znz.compass.znzlibray.views.recyclerview;
...
import com.znz.compass.znzlibray.views.recyclerview.animation.AlphaInAnimation;
import com.znz.compass.znzlibray.views.recyclerview.animation.BaseAnimation;
import com.znz.compass.znzlibray.views.recyclerview.animation.ScaleInAnimation;
import com.znz.compass.znzlibray.views.recyclerview.animation.SlideInBottomAnimation;
import com.znz.compass.znzlibray.views.recyclerview.animation.SlideInLeftAnimation;
import com.znz.compass.znzlibray.views.recyclerview.animation.SlideInRightAnimation;
import com.znz.compass.znzlibray.views.recyclerview.entity.IExpandable;
import com.znz.compass.znzlibray.views.recyclerview.loadmore.LoadMoreView;
import com.znz.compass.znzlibray.views.recyclerview.loadmore.SimpleLoadMoreView;
import com.znz.compass.znzlibray.views.recyclerview.util.MultiTypeDelegate;
...
public abstract class BaseQuickAdapter<T, K extends BaseViewHolder> extends RecyclerView.Adapter<K> {
    ...
    public void loadMoreEnd(boolean gone) {
        ...
        if (gone) {
            notifyItemRemoved(getHeaderLayoutCount() + mDataList.size() + getFooterLayoutCount());
        } else {
            mLoadMoreView.setLoadMoreStatus(LoadMoreView.STATUS_END);
            notifyItemChanged(getHeaderLayoutCount() + mDataList.size() + getFooterLayoutCount());
        }
    }
    ...
    public void setEnableLoadMore(boolean enable) {
        ...
        if (oldLoadMoreCount == 1) {
            ...
            if (newLoadMoreCount == 1) {
                mLoadMoreView.setLoadMoreStatus(LoadMoreView.STATUS_DEFAULT);
                notifyItemInserted(getHeaderLayoutCount() + mDataList.size() + getFooterLayoutCount());
            }
        }
    }
     ...
     * some initialization dataList.
     *
     * @param layoutResId The layout resource id of each item.
     * @param dataList    A new list is created out of this one to avoid mutable list
     */
    public BaseQuickAdapter(@LayoutRes int layoutResId, @Nullable List<T> dataList) {
    ...
    public BaseQuickAdapter(@Nullable List<T> dataList) {
        this(0, dataList);
    }
     ...
     * @param position
     */
    protected void getCurrentBean(int position) {
        bean = mDataList.get(position - getHeaderLayoutCount());
    }
    ...
    /**
     * add one new dataList in to certain location
     *
     * @param position
     */
    public void addData(@IntRange(from = 0) int position, @NonNull T dataList) {
        mDataList.add(position, dataList);
        notifyItemInserted(position + getHeaderLayoutCount());
        compatibilityDataSizeChanged(1);
    }

    /**
     * add one new dataList
     */
    public void addData(@NonNull T dataList) {
        mDataList.add(dataList);
        notifyItemInserted(mDataList.size() + getHeaderLayoutCount());
        compatibilityDataSizeChanged(1);
    }
     ...
     * change dataList
     */
    public void setData(@IntRange(from = 0) int index, @NonNull T dataList) {
        mDataList.set(index, dataList);
        notifyItemChanged(index + getHeaderLayoutCount());
    }

    /**
     * add new dataList in to certain location
     *
     * @param position
     */
    public void addData(@IntRange(from = 0) int position, @NonNull List<T> dataList) {
        mDataList.addAll(position, dataList);
        notifyItemRangeInserted(position + getHeaderLayoutCount(), dataList.size());
    ...
    public void addData(@NonNull List<T> newData) {
        ...
        notifyItemRangeInserted(mDataList.size() - newData.size() + getHeaderLayoutCount(), newData.size());
     ...
     *
     * @param size Need compatible dataList size
     */
    private void compatibilityDataSizeChanged(int size) {
        final int dataSize = mDataList == null ? 0 : mDataList.size();
        if (dataSize == size) {
            notifyDataSetChanged();
        }
    }
    ...
    /**
     * Get the dataList item associated with the specified position in the dataList set.
     *
     * @param position Position of the item whose dataList we want within the adapter's
     *                 dataList set.
     * @return The dataList at the specified position.
     */
    @Nullable
    ...
    public int getItemCount() {
        ...
        if (getEmptyViewCount() == 1) {
            ...
            count = getHeaderLayoutCount() + mDataList.size() + getFooterLayoutCount() + getLoadMoreViewCount();
        }
        return count;
    }
    ...
    protected int getDefItemViewType(int position) {
        if (mMultiTypeDelegate != null) {
            return mMultiTypeDelegate.getDefItemViewType(mDataList, position);
        }
    ...
    public void onBindViewHolder(K holder, int positions) {
        ...
        switch (viewType) {
            case 0:
                ...
                convert(holder, mDataList.get(holder.getLayoutPosition() - getHeaderLayoutCount()));
                break;
            case LOADING_VIEW:
        ...
        view.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                if (getOnItemClickListener() != null) {
                    getCurrentBean(baseViewHolder.getLayoutPosition());
    ...
    protected K createBaseViewHolder(ViewGroup parent, int layoutResId) {
        mDataManager = DataManager.getInstance(parent.getContext());
    ...
    protected K createBaseViewHolder(View view) {
        ...
        return null != k ? k : (K) new BaseViewHolder(view, mContext);
    }
    ...
    private int getFooterViewPosition() {
        ...
        if (getEmptyViewCount() == 1) {
            ...
            return getHeaderLayoutCount() + mDataList.size();
        }
        return -1;
    }
     ...
     *
     * @param isHeadAndEmpty false will not show headView if the dataList is empty true will show emptyView and headView
     */
     ...
     * that no dataList is available in this AdapterView.
     *
     ...
     *
     * @param firstOnly true just show anim when first loading false show anim when load the dataList every time
     */
     ...
     * @param bean   The item that needs to be displayed.
     */
    protected abstract void convert(K helper, T bean);
     ...
     *
     * @param position The position of the item within the adapter's dataList set whose row id we want.
     ...
     * if the item is not exist in the dataList list, return a negative.
     */
